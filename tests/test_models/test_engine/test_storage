...
class TestDBStorage(unittest.TestCase):
    ...
    def test_new_and_save(self):
        ...
        d = MySQLdb.connect(user=os.getenv('HBNB_MYSQL_USER'),
                             host=os.getenv('HBNB_MYSQL_HOST'),
                             passwd=os.getenv('HBNB_MYSQL_PWD'),
                             port=3306,
                             d=os.getenv('HBNB_MYSQL_DB'))
        new_use = User(**{'first_name': 'jack',
                           'last_name': 'bond',
                           'email': 'jack@bond.com',
                           'password': 12345})
        curr = d.cursor()
        curr.execute('SELECT COUNT(*) FROM users')
        old_count = curr.fetchall()
        curr.close()
        d.close()
        new_use.save()
        d = MySQLdb.connect(user=os.getenv('HBNB_MYSQL_USER'),
    ...
    def test_delete(self):
        ...
        self.assertIn('John', res)
        self.assertIn('Zoldyck', res)
        self.assertIn(object_key, storage.all(User).keys())
        new.delete()
        self.assertNotIn(object_key, storage.all(User).keys())
        cursor.close()
        dbc.close()
